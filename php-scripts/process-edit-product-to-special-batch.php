...
if (isset($_POST['batch_id']) && !empty($_POST['batch_id']) && isset($_POST['product_id']) && !empty($_POST['product_id']) && isset($_POST['single_case_price']) && !empty($_POST['single_case_price'])){ //condition 1
	...
	$stmt = $link->prepare('UPDATE `special_batch_products` SET  `single_case_price` = ?, `minimum_qty` = ?, `free_cases` = ? WHERE `id` = ? AND `clientid` = ? AND `uid` = ?');
	$stmt->bind_param('ssssss', $single_case_price, $minimum_qty, $free_cases, $batch_id, $clientid, $product_id);
	$stmt->execute();
