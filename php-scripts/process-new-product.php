...
if (isset($_POST['push_reason']) && !empty($_POST['push_reason'])){$push_reason= $_POST['push_reason'];}
	...
	$stmt = $link->prepare("INSERT INTO `grocery_products` (`uniqueid`, `upc`, `case_barcode`, `cert_code`, `size_unit`, `description`, `Pack`, `size_amount`, `QtyOnHand`, `package`, `normal_price`, `case_price`, `cost`, `case_cost`, `weight_case`, `weight_unit`, `memo`, `supplier`, `supplier_code`, `department`, `sub_department`, `category`, `brand`, `active`, `tax_id`, `lowest_allowed`, `highest_allowed`,`cases_on_pallet`,`on_special`, `special_ongoing`, `special_start`,`special_end`,`special_price`, `push_item`, `push_reason`, `clientid`,`created_by`, `date_created`) VALUES (UUID(),?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,'yes',?,?,?,?,?,?,?,?,?,?,?,?,?, NOW())");
	$stmt->bind_param('sssssssssssssssssssssssssssssssssss',$upc,$case_barcode,$cert_code,$size_unit,$description,$Pack,$size_amount,$QtyOnHand,$package,$normal_price,$case_price,$cost,$case_cost,$weight_case,$weight_unit,$memo,$supplier,$supplier_code,$department,$sub_department,$category,$brand, $tax_id, $lowest_allowed, $highest_allowed,$cases_on_pallet, $on_special, $special_ongoing, $special_start, $special_end, $special_price, $push_item, $push_reason, $clientid, $_SESSION['user']);
	$stmt->execute();
