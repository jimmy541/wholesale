...
if(isset($_GET['customer']) && !empty($_GET['customer'])){
	...
	$this->Cell(80);
    $this->Cell(90,5,'Account History',0,0,'L');
...
function FancyTable($link, $clientid, $hashed_customer_number, $pageNumber)
{
	...
	$w = array(25, 90, 37, 38);
	$header = array('Date', 'Description', 'Amount', 'Balance');
	for($i=0;$i<count($header);$i++)
		$this->Cell($w[$i],7,$header[$i],1,0,'C');
	$this->Ln();
	...
	$query = "SELECT `date`, `description`, `amount`, `type` FROM `temp_account_history` WHERE `clientid` = '$clientid' AND `customer_hash` = ? AND `date` >= '$datefrom' AND `date` <= '$dateto' ORDER BY `date` ASC $limit_clause";
					...
					$stmt->bind_param('s', $hashed_customer_number);
					$stmt->execute();
					$stmt->bind_result($dt, $desc, $amnt, $type);
					$x = 0;
					while($stmt->fetch()){
						$x++;
						
						$this->Cell($w[0],6,htmlspecialchars($dt),'LR',0,'L');
						$this->Cell($w[1],6,htmlspecialchars($desc),'LR',0,'L');
						$this->Cell($w[2],6,number_format($amnt, 2),'LR',0,'R');
						if($type == 'plus'){
							$running_balance += $amnt;
						}elseif ($type == 'minus'){
							$running_balance -= $amnt;
						}
						$this->Cell($w[3],6,number_format($running_balance, 2),'LR',0,'R');
						$this->Ln();
					}
...
if(isset($_GET['datefrom']) && validateDate2($_GET['datefrom']) && isset($_GET['dateto']) && validateDate2($_GET['dateto'])){
	$datefrom = $_GET['datefrom'];
	$dateto = $_GET['dateto'];
}
$query = "SELECT `date`, `description`, `amount`, `type` FROM `temp_account_history` WHERE `clientid` = '$clientid' AND `customer_hash` = ? AND `date` >= '$datefrom' AND `date` <= '$dateto'";
$stmt = $link->prepare($query);
$stmt->bind_param('s', $hashed_customer_number);
$stmt->execute();
$stmt->store_result();
...
if(isset($_GET['d']) && $_GET['d'] == '1'){
	$pdf->Output('d', 'Account History.pdf');
}elseif(isset($_GET['s']) && $_GET['s'] == '1'){
	$filename="temp_pdf_invoices/history$hashed_customer_number.pdf";
